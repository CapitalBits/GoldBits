...
uint256 hashGenesisBlock("0x4388e6d6250edb276668683b54305862ef3d8c10876bf0f81359939cef396bd2");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Goldbit: starting difficulty is 1 / 2^12
...
{
    // Goldbit: IsDust() detection disabled, allows any valid dust to be relayed.
...
//    int64 nSubsidy = 50 * COIN;
    ...
    //nSubsidy >>= (nHeight / 840000); // Goldbit: 840k blocks in ~4 years
    
    if(nHeight > 1500) // no block reward after 1500 blocks
        nSubsidy = 0;
    
    return nSubsidy + nFees;
}

//static const int64 nTargetTimespan = 3.5 * 24 * 60 * 60; // Goldbit: 3.5 days
//static const int64 nTargetSpacing = 2.5 * 60; // Goldbit: 2.5 minutes
static const int64 nTargetTimespan = 0.25 * 24 * 60 * 60; // Goldbit: 0.25 days
static const int64 nTargetSpacing = 30; // Goldbit: 30s
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Goldbit: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Goldbit: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // Goldbit: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "EET 13/05/2014 The new future has begun";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1405658008;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2087553738;
        ...
        assert(block.hashMerkleRoot == uint256("0x7c28bea3ce449d8c9c0d7d045a6a70a265e906351a5639dd58ded3bdcc7dcb9a"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // Goldbit: increase each by adding 2 to bitcoin's value.
